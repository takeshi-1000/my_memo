...
コンピュータの初期には、処理できる最大量がcpuのクロック速度に決まっていたが、プロセッサがよりコンパクトな設計になるにつれ、物理的な制約からクロック速度を高めることができず小さくなってしまった。（クロック速度高めすぎて、発熱量が増える
そこで各チップメーカーが考え出した概念が、マルチコアというもので、物理的な制約はそのままに一つのcpuが実行する処理の単位（コア）を増やし、1秒当たりの命令量を増やすという考え方だった。

## 並行、並列とか
https://github.com/takeshi-1000/my_memo/blob/main/2022/0714_%E4%B8%A6%E8%A1%8C_%E4%B8%A6%E5%88%97_%E3%82%B9%E3%83%AC%E3%83%83%E3%83%89%E3%81%AA%E3%81%A9.md あたりに記載
...
- 任意のスレッドで実行したい処理を追加して、スレッド自体の作成やそのタスクを割り当てられたスレッドで実行するような管理とかは、GCD側のランタイムコードでよしなにやってくれること
- 同期/非同期イメージ
...
- https://developer.apple.com/videos/play/wwdc2016/72
  - コードを実行するキューの適切な数を選択すること
  - メインスレッドから別のキューにタスクを追加するには、アプリケーション内の独立したデータフロー（例えば画像変換やデータベースなど）を持つ領域を特定し、それらサブシステムに独立して作業を実行するためのキューを割り当てると、スレッド爆発は起こらない
  - 異なるキューで処理を実行する、ということを連鎖しながら実行する方法が基本的な使い方っぽいが、異なる複数のキューで処理を実行した後、それらが全て終わった後に任意のスレッドに通知させる、DispatchGroupという考え方もあるっぽい
