...
- git rebase はcommitのハッシュ値が変更するので、mainやdevelopブランチなどにいる状態で、使うのはやや危険そう
- pull requestで、本流のブランチにマージをする
- そして、本流へのブランチに対する変更のブランチで（developから切ったfeatureブランチなど）、レビューをしやすくしてもらうという意味で、git rebase するのは良さそう
